...
func (b *Builder) RenderChainMethod(file *File, method Method) {
	...
	file.L(`func (` + b.ReceiverName() + ` ` + b.Type.Obj().Name() + `) ` + method.Alias + `(` + strings.Join(inputParams, ", ") + `) ` + b.Type.Obj().Name() + " {")
...
func (b *Builder) RenderFinalizer(file *File, method Method) {
	...
	ref := b.ReceiverName() + "." + b.Ref(method.Builder).Name
	file.L(`func (` + b.ReceiverName() + ` ` + b.Type.Obj().Name() + `) ` + method.Alias + `(` + strings.Join(inputParams, ", ") + `) ` + outputParamsStr + " {")
	file.P("\t")
...
func (m Method) Doc() *ast.CommentGroup {
	...
	fileTok := pkg.Fset.File(fileScope.Pos())
...
func (g Glob) Match(typeName string, str string) bool {
	...
	match := true
	if g.Prefix != "" {
		match = match && strings.HasPrefix(str, g.Prefix)
	}
	if g.Suffix != "" {
		match = match && strings.HasSuffix(str, g.Suffix)
	}
	return match
}
